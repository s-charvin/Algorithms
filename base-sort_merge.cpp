...
void merge(int array[], int const left, int const mid,
           int const right)
{
    auto const subArrayOne = mid - left + 1; // 计算输入 array 的左侧数据数量(包含中点)
    auto const subArrayTwo = right - mid;    // 计算输入 array 的右侧数据数量(不包含中点)
    ...
    int indexOfMergedArray = left; // 初始化输入 array 的起始 index.
...
void mergeSort(int *array, uint64_t begin, uint64_t end)
{
    if (begin >= end) // 当递归到输入 array 只有一个值时
        return;       // 控制递归结束

    auto mid = begin + (end - begin) / 2; // 计算输入 array 的中点位置
    mergeSort(array, begin, mid);         // 左段计算
    mergeSort(array, mid + 1, end);       // 右段计算
    merge(array, begin, mid, end);        // 正向递归结束, 开始逐步返回, 融合每段结果
}
int main()
{
    // 获取输入容器所含的元素数量
